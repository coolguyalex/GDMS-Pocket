...
void drawCategories() {
  ...
  const int rows   = 4;     // 4 rows => 20..51, footer at 54 is clear
...
void drawTable() {
  display.clearDisplay();
  drawHeader(selectedFile.c_str());   // header shows CSV filename
  ...
  if (currentEntry.length() == 0) {
    ...
    drawFooterWithCounter("A:reroll  B:back", 0, 0);
    display.display();
    return;
  }

  // Print the entry starting below header.
  // Simple wrap: we’ll print characters and move to next line.
  const int x0 = 0;
  int x = x0;
  int y = 20;
  const int lineH = 8;
  const int maxY = 52 - lineH; // keep clear of footer line
  ...
  for (uint16_t i = 0; i < currentEntry.length(); i++) {
    ...
    display.write(c);
    x += 6; // approx width of size-1 font char
  }

  // Footer: show rollCount as "pos/total" isn't meaningful here, so we show roll count as 1-based.
  // We'll reuse footer helper by faking "pos/total" = rollCount/rollCount
  // so it displays "N/N". If you prefer "roll:N" we can change next.
  drawFooterWithCounter("A:reroll  B:back", (rollCount > 0 ? rollCount - 1 : 0), (rollCount > 0 ? rollCount : 0));
...
void onButtonPressed(BtnId b) {
  ...
  } else if (b == BTN_B) { // back to file list
    mode = MODE_FILES;
  }
}
