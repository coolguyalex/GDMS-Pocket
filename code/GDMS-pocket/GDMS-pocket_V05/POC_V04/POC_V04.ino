...
// =================== SD CONFIG ===================
// Keep SdFat for now (your code uses FsFile + fgets).

// Choose wiring profile:
// 1 = Feather RP2040 Adalogger built-in microSD (uses SPI1 + CS 23)
// 0 = External SPI microSD module (set CS pin + SPI bus below)
#define SD_WIRING_ADALOGGER 1

#if SD_WIRING_ADALOGGER
  #define SD_CS_PIN   23
  #define SD_SPI_BUS  SPI1
  #define SD_SPI_MHZ  16
#else
  // Example defaults for an external module (change to match your wiring)
  #define SD_CS_PIN   5       // <-- pick your CS GPIO
  #define SD_SPI_BUS  SPI     // <-- SPI or SPI1 depending on which pins you wire
  #define SD_SPI_MHZ  12      // safer for jumper wires; can raise later
#endif

SdFat SD;

bool sdInit() {
  SdSpiConfig cfg(SD_CS_PIN, DEDICATED_SPI, SD_SCK_MHZ(SD_SPI_MHZ), &SD_SPI_BUS);
  return SD.begin(cfg);
}
