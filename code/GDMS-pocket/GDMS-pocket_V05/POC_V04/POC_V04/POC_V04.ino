...
String stripExt(const String& s) {
  if (s.length() >= 4) {
    String tail4 = s.substring(s.length() - 4);
    tail4.toLowerCase();
    if (tail4 == ".csv") return s.substring(0, s.length() - 4);
  }
  if (s.length() >= 5) {
    String tail5 = s.substring(s.length() - 5);
    tail5.toLowerCase();
    if (tail5 == ".json") return s.substring(0, s.length() - 5);
  }
  return s;
}
...
bool chancePass(float chance01) {
  if (chance01 >= 1.0f) return true;
  if (chance01 <= 0.0f) return false;

  // random(0,10000) => 0..9999
  uint32_t r = (uint32_t)random(0, 10000);
  uint32_t t = (uint32_t)(chance01 * 10000.0f);
  return r < t;
}

String dirOfPath(const String& fullPath) {
  int slash = fullPath.lastIndexOf('/');
  if (slash < 0) return "";
  return fullPath.substring(0, slash + 1); // includes trailing /
}

// Builds a multi-line string like:
// "Size: Large\nShape: Oval\nTrap: Poison needle\n"
bool runRecipeToEntry(const String& recipePath, String& outEntry) {
  outEntry = "";

  File f = SD.open(recipePath.c_str(), FILE_READ);
  if (!f) return false;
  ...
  // Seed RNG once per recipe run (good enough)
  randomSeed(micros());

  String baseDir = dirOfPath(recipePath);

  for (JsonObject step : steps) {
    const char* labelC = step["label"] | "";
    const char* tableC = step["table"] | "";
    int rolls = step["rolls"] | 1;
    float chance = step["chance"] | 1.0f;

    if (!labelC[0] || !tableC[0]) continue;
    if (!chancePass(chance)) continue;
    if (rolls < 1) rolls = 1;

    // Resolve table path:
    // - If it starts with "/", treat as absolute on SD
    // - Else, treat as relative to the recipe’s folder
    String tablePath = String(tableC);
    if (!tablePath.startsWith("/")) {
      tablePath = baseDir + tablePath;
    }

    String combined = "";
    for (int i = 0; i < rolls; i++) {
      String one = "";
      if (pickRandomCsvLine(tablePath, one)) {
    ...
    outEntry += String(labelC) + ": " + combined + "\n";
  }

  if (outEntry.length() == 0) outEntry = "No steps triggered.";
  return true;
}
...
void drawFiles() {
  ...
  for (int r = 0; r < rows; r++) {
    ...
    String shown = stripExt(files[idx]);
...
void drawTable() {
  ...
  String hdr = stripExt(selectedFile);
  ...
  if (mode == MODE_CATS) {
    ...
    if (b == BTN_UP) fileCursor--;
      ...
      if (fileCount > 0) {
        ...
        if (endsWithJson(selectedFile)) {
          ok = runRecipeToEntry(fullPath, currentEntry);
        } else {
        ...
        if (!ok) {
          Serial.println("Failed to load selection.");
          currentEntry = "Failed to load selection.";
        }
      }
